...
(function (S) {
  ...
  S.login = function (q) {
    ...
    if (tab === 'join') {
      ...
      '<div class="sec" style="padding-top:0"><button class="btn kakao" data-act="do-login">카카오로 로그인</button></div>';
    }
  ...
  S.verify = function () {
    return { bar: { title: '가입 심사', back: true, center: true }, tab: false, html:
      ...
      '<div class="field"><label>1. 현재 혼인 상태</label><div class="opts" data-radio="marital">' +
        ['이혼','사별','사실혼 종료'].map(o => '<button class="opt" data-val="' + o + '">' + o + '</button>').join('') +
      '</div><div class="hint">이용 자격 확인용입니다. 프로필 공개 여부는 직접 선택할 수 있어요.</div></div>' +

      '<div class="field"><label>2. 인증 방법</label><div class="opts" data-radio="method">' +
        ['혼인관계증명서','운영자 확인'].map(o => '<button class="opt" data-val="' + o + '">' + o + '</button>').join('') +
      '</div></div>' +

      '<div class="field"><label>증빙 파일 첨부</label>' +
        '<button class="uploader" data-act="upload"><b>＋</b>파일 선택 또는 사진 촬영</button>' +
        '<div class="hint">주민등록번호·주소는 자동으로 가려집니다. 원본은 심사 후 보관기한에 따라 파기합니다.</div></div>' +

      '<div class="field"><label>3. 가입 질문</label>' +
        '<textarea class="inp" id="f-answer" placeholder="가입 목적과 원하는 활동을 작성해 주세요. (최소 50자)"></textarea>' +
        '<div class="hint"><span id="cnt">0</span>/50자 · 욕설, 연락처, SNS 아이디가 포함되면 보완 요청을 받을 수 있어요.</div></div>' +
      ...
      after: function () {
        ...
        if (ta) ta.addEventListener('input', () => { c.textContent = ta.value.trim().length; });
      } };
  };
  ...
  S.review = function () {
    return { bar: { title: '심사 진행 중', back: true, center: true }, tab: false, html:
      '<div class="hero-state"><div class="hero-ic">⏳</div>' +
      '<h3>가입 심사가 접수되었습니다</h3>' +
      '<p>접수 순서대로 운영자가 확인하고 있어요.<br>보완이 필요하면 알림으로 안내드립니다.</p></div>' +
      '<div class="list">' +
  ...
  function profileForm (s, isEdit) {
    const vis = k => '<button class="visbtn" data-vis="' + k + '">' + esc(s[k] || '비공개') + ' ▾</button>';
    return '' +
    '<div class="sec" style="text-align:center;padding-top:22px">' +
      '<button class="photo" data-act="photo">' + UI.initial(s.nickname) + '<span>＋</span></button>' +
