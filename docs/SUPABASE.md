...
지금 웹앱은 서버 없이 브라우저 안에서만 동작합니다. 그래서 채팅도 **내가 보낸 메시지만 내 기기에 쌓일 뿐**,
...
프로젝트 생성 시 `Automatically expose new tables`를 꺼도 괜찮습니다. SQL이
`authenticated` 역할에 채팅에 필요한 `SELECT·INSERT·DELETE` 권한만 직접 부여합니다.
...
폰과 PC 브라우저에서 `대화 → 채팅`을 엽니다. 두 기기 모두 별도의 방 선택 없이 `global` 전체채팅 채널에 접속합니다.
